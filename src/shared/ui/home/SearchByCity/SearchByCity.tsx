import CrossIcon from '@/public/images/SvgIcons/CrossIcon.svg'
...
const SearchByCity = ({ inputError }: SearchByCityProps) => {
    const router = useRouter()
    const [searchCity, setSearchCity] = useState('')

    console.log(inputError, 'inputtttt')
    ...
    return (
        <div>
            <div className='flex gap-6 border-b-[1px] border-border_color_grey pb-4 mb-6'>
                <div
                    className={`bg-grey border border-border_color_grey rounded-lg flex items-center w-full h-full py-3 pl-3 relative                     ${
                        inputError ? 'border-red' : 'border-blue'
                    }`}
                >
                    <label
                    ...
                    <div className='absolute flex items-center h-full right-4 top-0 bottom-0'>
                        <button
                            className=' px-2 h-[30px] default-hover-active'
                            type='button'
                            onClick={clearCity}
                        >
                            <CrossIcon />
